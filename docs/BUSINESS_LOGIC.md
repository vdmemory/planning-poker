...
Каждый emoji-флоатер рендерит `<video>` с анимированным MP4 из `frontend/public/reactions/<codepoint>.mp4`. Маппинг живёт в `REACTION_EMOJI_VIDEO` рядом со списком эмоджи. Если для эмоджи нет файла (например, peer на старой сборке прислал не-маппленную реакцию), флоатер fallback'ится на текстовый глиф — UX мягко деградирует. Time-values никогда не идут через видео — это лейблы-пилюли.

Видео — `autoplay muted loop playsInline preload="auto"`. `muted` обязателен, иначе браузеры не разрешат autoplay без user gesture; `loop` нужен потому что `REACTION_FLOATER_MS = 3500ms` длиннее самих клипов (~1-2 с).

Overlay над карточкой остаётся текстовым эмоджи — он маленький и эфемерный, видео там было бы избыточно.
