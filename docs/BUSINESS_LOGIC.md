...
На **мобиле** та же самая измеряемая позиция, наложенная на маленький экран, физически не помещалась и на первый кадр показывала пикер unpositioned (`style={}`) прямо в потоке документа, а затем дёргала его в вычисленную позицию — заметный «прыжок» при первом открытии. Поэтому для `< md` рядом с тем же самым (одним) `btnRef`-триггером рендерится второй вариант — центрированная модалка без каких-либо измерений (`fixed inset-0 flex items-center justify-center bg-black/50`, тот же паттерн что уже использует `RevotePicker` на главном экране: карточка игрока → карандаш → смена голоса). Оба варианта (`md:hidden` модалка / `hidden md:block` дропдаун) смонтированы одновременно и переключаются CSS-брейкпоинтом — тот же приём, что и `PokerTable`/`ActionBox` в `RoomPage.tsx` (см. вводный абзац этого раздела).
