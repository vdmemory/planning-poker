...
> Этот файл — про **Planning Poker**. Для **Retro Board** (issue #62, отдельный независимый продукт на том же сайте) — см. `docs/RETRO_BUSINESS_LOGIC.md`.
...
### Наклон карандашного курсора (issue #50)

Сам курсор-карандаш — SVG (`path="M16 2l4 4-11 11H5v-4L16 2z"`, viewBox `0 0 22 22`), позиционируемый через `left`/`top` относительно `fixed`-обёртки, которая уже сидит на `cursorPos` (реальных координатах мыши). Вершина `(20,6)` этого пути — «пишущий» кончик (острая грань `(16,2)-(20,6)`); вершины `(5,17)`/`(5,13)` — плоский срез «ластика».

До фикса `transform: rotate(-45deg)` был без `transform-origin` (т.е. вращение вокруг центра бокса `11,11`), из-за чего кончик и точка вращения не совпадали — под этим конкретным углом расхождение было небольшим и визуально терпимым, но подобрать `left`/`top` для любого ДРУГОГО угла вручную значило бы каждый раз пересчитывать оффсеты заново.

Фикс: `transformOrigin: "20px 6px"` — пин вращения ровно на вершину-кончик пера. Вращение вокруг собственной точки никогда её не двигает, так что `left: -20, top: -6` (позиция кончика при отсутствии поворота) остаётся верной для любого угла — сам угол (`rotate(100deg)`) можно менять свободно, не трогая оффсеты. `100deg` даёт наклон вправо ~35° от вертикали (кончик снизу-справа, ластик сверху-слева) — в требуемом issue диапазоне 30–45°. Формула для другого угла наклона `t` (в градусах, вправо от вертикали): `rotate = 135 - t`.

Проверено через `getBoundingClientRect()` временного SVG-маркера на вершине `(20,6)` при известной позиции мыши — кончик рендерится в пределах <1px от курсора; e2e-тест (`drawing-toggle-and-fade.spec.ts`) закрепляет то же самое.
...
Чисто фронтенд-полировка, без нового WS-протокола и без бизнес-правил — CSS keyframes в `index.css` + минимум JS-состояния, тот же lightweight-подход, что и `throw-reaction-land`/`reaction-overlay-pop` (issue #51/#32), без анимационной библиотеки (issue явно просил не тянуть тяжёлые зависимости).

### Переворот карты при reveal

`PlayerCard` держит локальный `flipping`-стейт: `useEffect` на пропе `revealed` ловит именно переход `false → true` (не каждый ре-рендер, где `revealed` уже `true` — иначе голосование другого игрока или смена ника переигрывали бы флип), включает класс `card-flip` (`@keyframes card-flip`, `rotateY`) на карточке и снимает его через 400ms таймером. Один и тот же эффект переигрывается на новый раунд, потому что `revealed` перед этим успевает стать `false` (`reset`).

### Fade-in / fade-out игроков (`usePlayersWithExit`)

Джойн: обёртка каждого игрока в `PokerTable` всегда несёт класс `player-fade-in` — CSS-анимация сама проигрывается один раз при монтировании нового DOM-узла (`key={player.id}`), ре-плей на обычных ре-рендерах не нужен, класс не снимается.

Уход — сложнее, потому что `state.players` присылает уже урезанный список, а «показать что-то, чего больше нет» без анимационной библиотеки требует явно придержать удалённого игрока. Хук `usePlayersWithExit` (`frontend/src/hooks/usePlayersWithExit.ts`) сравнивает предыдущий и новый список игроков по id, и любого, кто выпал, ещё `EXIT_MS = 260ms` держит в отдельном `leaving`-стейте («ghost»). `PokerTable` рендерит ghost'ов отдельным проходом поверх орбиты с классом `player-fade-out` (`data-testid="player-card-ghost"`, `pointer-events-none`, чтобы с угасающей карточкой нельзя было взаимодействовать).

Поскольку позиция каждого игрока на овале — функция от `n = players.length` и его индекса, у выбывшего игрока в новом `n` просто нет места. Чтобы ghost не прыгал в случайную точку, `PokerTable` кеширует последнюю вычисленную `{x, y}` каждого активного игрока в `lastPosRef` на каждом рендере; ghost берёт из этого кеша свою последнюю позицию перед уходом. Позиции ОСТАЛЬНЫХ игроков при этом не переанимируются — они просто скачком занимают новую раскладку овала в момент, когда ghost окончательно исчезает (это осознанный компромисс: FLIP для всей орбиты — отдельная, более дорогая фича, не то, что просил issue).

### Slide-in статистики (`TableCenter`)

Ветки `TableCenter` (countdown / stats / everyone-voted / voted-count) исторически не имели `key`, поэтому React переиспользовал один и тот же корневой `div` между ветками — CSS-анимация на нём проигралась бы только один раз за всё время жизни компонента. Ветка со stats получила `key="stats"` — при переключении на неё (из любой другой, все они без явного key, то есть `undefined !== "stats"`) React гарантированно монтирует новый DOM-узел, и `stats-slide-in` проигрывается на каждый `reveal`, а не только на первый.

### Побочный фикс: `countdownPop` ничего не делал

Обратный отсчёт («Revealing in… 3») уже ссылался на анимацию `style={{animation: "countdownPop 0.3s ease-out"}}`, но keyframe `countdownPop` нигде не был определён — число менялось (за счёт `key={countdown}`, реально работающего ремаунта), но сам pop-эффект никогда не проигрывался. Добавлен вместе с остальными keyframe'ами issue #5, раз уж он логически из той же категории.

### Reorder issues — FLIP (`IssueSidebar`)

`reorder_issue` (top/up/down/bottom) и так работал — просто без анимации, строки перескакивали на новую позицию мгновенно. `IssueSidebar` следит за `state.issues` через `useLayoutEffect`: перед тем как обновить `prevRectsRef`, если порядок id изменился, для каждой строки (найденной по `data-issue-id`, добавлен на `<li>` в `IssueCard`) считает дельту между старой и новой `getBoundingClientRect().top`, синхронно проставляет `transform: translateY(delta)` без transition, форсит reflow (`el.getBoundingClientRect()`), и на следующем кадре включает `transition: transform 0.25s` и сбрасывает `transform` — ручной FLIP, тот же приём, что `ThrowFloater` (issue #51), без сторонней библиотеки.

### Тесты

`frontend/tests/e2e/animations.spec.ts` — проверяет присутствие/своевременное снятие классов (`card-flip`, `player-fade-in`, `player-fade-out` + `data-testid="player-card-ghost"` исчезает, `stats-slide-in`), а также что `data-issue-id` есть на строках и reorder по-прежнему меняет видимый порядок. Точный тайминг transform в середине FLIP-анимации не проверяется — слишком хрупко для e2e, полагаемся на функциональную проверку итогового порядка.
