...
### Estimate picker без прыжка (issue #23 follow-up)

Пикер оценки в `IssueSidebar.tsx` (кнопка с текущей оценкой на карточке issue) раньше был fixed-position дропдауном: измерял `getBoundingClientRect()` кнопки-триггера в `useEffect` **после** первого рендера и только тогда проставлял `position: fixed` через `style` state. На первый кадр стиль пустой (`{}`), поэтому пикер на мгновение рендерился в потоке документа как обычный блок, а затем дёргался в вычисленную позицию — заметный «прыжок» при первом открытии. Заменён на тот же паттерн, что уже использует `RevotePicker` на главном экране (карточка игрока → карандаш → смена голоса): `fixed inset-0 flex items-center justify-center bg-black/50` центрированная модалка без каких-либо измерений — координаты не нужны, флексбокс центрирует сам.
