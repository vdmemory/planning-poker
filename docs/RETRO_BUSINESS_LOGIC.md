...
## Вложения к карточкам: эмодзи, GIF, изображения (issue #66)

Возможность прикрепить эмодзи, GIF или картинку при создании/редактировании карточки. Разбито issue на два этапа (Этап A/B), реализовано одним PR:

- **Этап A — эмодзи-пикер**: чисто фронтовая фича, никаких изменений на бэке не требует — эмодзи это unicode-символы внутри уже существующего `RetroCard.text`. `RetroCardAttachmentPicker.tsx` — самописный попап (проект принципиально безбиблиотечный на UI, см. issue #22) с сеткой ~48 эмодзи; клик вставляет символ в позицию курсора текстового поля через общий хелпер `insertTextAtCursor.ts` (нужен `RefObject` на textarea + `selectionStart`/`selectionEnd`), а не всегда в конец текста.
- **Этап B — GIF и изображения**: `RetroCard.image_url: str | None` (`retro_models.py`) — **только внешний URL, никогда не бинарные данные**. Сознательное scope-решение из самого issue: проект принципиально in-memory, без БД и без файлового стораджа (см. `CLAUDE.md`), поэтому загрузка файлов на свой сервер осталась за рамками MVP — можно вставить GIF через поиск (GIPHY, внешние постоянные ссылки) или картинку по прямому `http(s)://` URL.

### GIF-поиск — серверный прокси, не прямой клиентский запрос

`GET /api/retro-boards/gif-search?q=...` (`gif_client.py:search_gifs`) проксирует к GIPHY REST API, держа `GIPHY_API_KEY` только на сервере — фронт никогда не видит ключ. Пустой `q` бьёт в GIPHY's `/v1/gifs/trending` вместо `/v1/gifs/search`, так что попап показывает наполненную сетку сразу при открытии таба, ещё до того как пользователь начал печатать.

**Роут зарегистрирован ДО `/api/retro-boards/{board_id}`** в `main.py` — Starlette матчит path-шаблоны в порядке регистрации, и если бы `{board_id}` шёл первым, он бы перехватил литеральный путь `gif-search` как `board_id="gif-search"` (и вернул 404 «доска не найдена», а не результаты поиска). Тот же класс бага, что уже встречался в этом проекте с REST-роутами Planning Poker/Retro Board — статичные пути должны регистрироваться раньше параметризованных.

Без `GIPHY_API_KEY` в Environment эндпоинт возвращает `503` с понятным сообщением — фронт показывает «GIF search is unavailable» в самом попапе, но эмодзи-пикер и вставка по прямому URL продолжают работать без единой настройки на бэке (не завязаны на ключ вовсе).

### Валидация

`RetroService._validate_image_url()` — не антивирус-проверка контента (не цель проекта, та же логика доверия, что и у текста карточек), только базовая защита от мусора: `urlparse(url).scheme` должен быть `http`/`https` и `netloc` не пустой — отклоняет `javascript:`, голые имена файлов и т.п. Пустая строка или `None` трактуются как «нет картинки», не как ошибка.

`edit_card` **не использует partial-update семантику** для `image_url` (в отличие от опциональных полей `update_board`) — фронт всегда пересылает текущее значение `image_url` карточки при каждом редактировании текста, так что отсутствие поля в WS-payload означает «убрать картинку». Такое решение выбрано потому что редактирование текста и картинки идут через одну и ту же форму/попап на фронте — нет отдельного экшена «просто отредактировать текст, не трогая картинку».

### Frontend

- `RetroCardAttachmentPicker.tsx` — два таба: «Emoji» (грид) и «GIF / Image» (поиск-по-мере-печати с debounce 300мс + поле для прямой ссылки). Общий компонент, используется и в композере новой карточки (`RetroColumn.tsx`), и в форме редактирования существующей (`RetroCardItem.tsx`).
- **Попап открывается вниз** (`top-full`, не `bottom-full`) — регрессия, найденная и исправленная при первом прогоне e2e: композер добавления карточки обычно сидит близко к верху короткой/пустой колонки, и открытие попапа ВВЕРХ у такой карточки залезало под фиксированный хедер (или под бар «Votes left» в режиме редактирования первой карточки колонки), из-за чего клики по табам «перехватывались» элементами хедера. Тот же принцип, что уже применялся для `RetroCardCommentThread`/старого `RetroCardReactionBar`.
- Превью прикреплённой картинки (до сабмита/сохранения) рендерится с фиксированной `h-24` (не `max-h-24`) — если URL окажется битым, блок всё равно занимает место, а не схлопывается в 0×0 (что вдобавок ловилось Playwright'ом как «invisible» в e2e).
- Отрисовка в самой карточке (`retro-card-image`, `RetroCardItem`/`RetroCardStack`) — `max-h-32`, `onError` прячет тег полностью, если картинка не грузится (не показываем «битую» иконку браузера).
- **Смёрженные карточки (`RetroCardStack`)**: показывается картинка только ГОЛОВЫ стопки — та же конвенция, что и с общим голосом/автором, по прямой рекомендации issue.
