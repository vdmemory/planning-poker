...
- **Слияние целых стопок**: если у `source_card_id` уже есть свои дочерние карточки (он сам — голова другой стопки), вся его стопка «переезжает» под голову `target_card_id` за один проход (`group_cards` резолвит head обеих сторон и перепривязывает всех, чей head совпадает с head'ом source, на head target'а).
...
Хук `useRetroCardDrag` (`frontend/src/hooks/useRetroCardDrag.ts`) на каждом `pointermove` вызывает `document.elementFromPoint(clientX, clientY)` чтобы определить карточку и колонку под курсором. **Кросс-колоночный drop отклоняется на клиенте** молча (без подсветки, без WS-сообщения) — сервер тоже отклонил бы (`group_cards` требует одной колонки), но полноэкранный error-оверлей `RetroBoardPage` не подходящее место для реакции на случайный неверный drop, только для настоящих багов.
...
И ручка перетаскивания (`data-testid="retro-card-grip"`), и панель реакций (`data-testid="retro-card-reaction-bar"`) следуют той же capability-based видимости, что и `ThrowReactionBar` в Planning Poker (issue #23/#51): `opacity-100` по умолчанию (нужно на тачскрине, где нет hover), скрывается через `[@media(hover:hover)]:opacity-0` + `group-hover:opacity-100` только на устройствах с настоящим указателем. Genuine touch-drag группировки не покрыт e2e-тестами (см. `docs/TESTING.md`) — синтетические `TouchEvent`, диспатченные тестом, не транслируются браузером в `PointerEvent` так, как это делает реальное аппаратное прикосновение; e2e использует настоящий `page.mouse` (который Chromium честно транслирует в `PointerEvent`) вместо этого.
