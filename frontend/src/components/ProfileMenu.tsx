...
export function ProfileMenu({
  ...
  return (
    // Issue #23 follow-up — on mobile this opens as a centered modal (same
    // treatment as GameSettingsModal) instead of the anchored dropdown below,
    // which had nowhere near enough room to breathe next to the avatar
    // button on a narrow screen. Desktop keeps the original dropdown; the
    // existing outside-mousedown listener above already closes it either way
    // since it just checks "was the click outside menuRef", regardless of
    // whether that's a page click (desktop) or a tap on the dimmed backdrop
    // (mobile).
    <div
    ...
    >
      ...
      >
        {/* Header — mobile modal only; desktop dropdown has no header */}
