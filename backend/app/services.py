...
# How long a new room stays active before it expires and is auto-closed.
# Module-level so tests can monkeypatch it to a short interval. Used by
...
class RoomService:
    ...
    def create_room(self, name: str, deck_type: DeckType, facilitator_nickname: str) -> tuple[Room, Player]:
        ...
        room = Room(name=name, deck_type=deck_type, expires_at=now + ROOM_LIFETIME)
    ...
    def get_room(self, room_id: str) -> Room:
        """Return an active room. Raises `RoomError` if missing or expired.

        Expired rooms are treated as not-existing for the purposes of any
        action; the cleanup task will remove them from the store and broadcast
        `room_expired` shortly after. By raising here we make every mutation
        method (vote, reveal, add_issue…) fail immediately on an expired room
        without having to add explicit checks in each.
        """
        room = self.store.get(room_id)
        if not room:
            raise RoomError(f"Room {room_id} not found")
        if room.is_expired():
            raise RoomError(f"Room {room_id} has expired")
        return room

    def expire_room(self, room_id: str) -> Optional[Room]:
        """Remove an expired room from the store. Returns the room (for the
        ws_manager to broadcast `room_expired` to remaining connections) or
        None if the room is already gone.
        """
        room = self.store.get(room_id)
        if not room:
            return None
